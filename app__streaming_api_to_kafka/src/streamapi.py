...
class streamAPI:
    ...
    async def aiosseclient(self, last_id=None, **kwargs):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async for line in response.content:
                ...
                try:
                    yield self.parse_football_stat(json.loads(line))
                except ValueError:
    ...
    def parse_football_stat(self, raw: dict = {}) -> dict:
        """
        parsing info about football events 
        """
        
        if raw == {}: return {}
        #---------------------------------------------------------------------
        elif isinstance(raw, dict) and 'data' in raw: return raw['data']
        #---------------------------------------------------------------------
        return {}
