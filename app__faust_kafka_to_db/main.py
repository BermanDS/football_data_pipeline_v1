...
async def process(transactions):
    async for value in transactions:
        ...
        if not all([fe.main_instance, fe.meta_instance]):
            ...
            continue
        #-----------------------------------------------------------------------------------
        if int(fe.data['event_id']) in [34151703,34151689]: continue
        ...
        #----------------------------------------------------------------------------------
        if fe.meta_instance:
            if db.insert_values(fe.data['competitions_matches'], f"{configs['PG__SCHEMA']}.competitions_matches"):
                None
            else:
        ...
        #------------------------------------------------------------------------------------
        if fe.match_actions():
            if db.insert_values(fe.data['match_actions'], f"{configs['PG__SCHEMA']}.match_actions"):
                None
            else:
        ...
        if fe.match_missed_goals():
            if db.insert_values(fe.data['match_missed_goals'], f"{configs['PG__SCHEMA']}.match_missed_goals"):
                None
            else:
        ...
        #-----------------------------------------------------------------------------------
        if fe.match_goals():
            if db.insert_values(fe.data['match_goals'], f"{configs['PG__SCHEMA']}.match_goals"):
                None
            else:
        ...
        #-----------------------------------------------------------------------------------
        if fe.match_breaks():
            if db.insert_values(fe.data['match_breaks'], f"{configs['PG__SCHEMA']}.match_breaks"):
                None
            else:
